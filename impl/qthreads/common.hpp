...
// helper for run_across
template <std::size_t I, typename Func, typename... Args>
void apply_at_index(Func &&func, Args &&...args) {
  static_cast<Func &&>(func)(std::get<I>(static_cast<Args &&>(args))...);
}

// helper for run_across
template <std::size_t... I, typename Func, typename... Args>
void run_across_impl(std::index_sequence<I...>, Func &&func, Args &&...args) {
  ...
  (apply_at_index<I>(func, args...), ...);
}

// run_across is like std::apply, but, given an invocable f and
// tuples of length n, calls f n times, pulling its arguments from
// each successive entry of the tuples.
...
void run_across(Func &&func, Args &&...args) {
  ...
  run_across_impl(std::make_index_sequence<tuple_size>(),
